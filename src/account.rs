...
pub fn codex_status(config: &Config) -> serde_json::Value {
    ...
    let active = default.as_deref().and_then(|default| {
        crate::openai::account::resolve_active_label(default, &config.openai.accounts)
    });
    let rows: Vec<serde_json::Value> = config
        ...
        .map(|account| {
            serde_json::json!({
                ...
                "signed_in": crate::openai::account::identity(account).is_some(),
...
fn status_lines(report: &serde_json::Value) -> Vec<String> {
    ...
    if !codex.is_empty() {
        out.push(String::new());
        out.push("Codex            ~/.codex/auth.json".to_string());
...
fn switch_codex(config: &Config, args: &SwitchArgs) -> i32 {
    ...
    let outcome = config.openai.resolve_auth_path(None).and_then(|default| {
        switch_account(
            &default,
...
fn add_codex(label: &str, login: bool, adopt: bool) -> i32 {
    ...
    if registration.already_existed {
        ...
        println!("  codex_auth_path = {}", registration.display);
    }
    println!();
    ...
    if adopt {
        let adopted = Config::load_from(&registration.config_path).and_then(|config| {
            let default = config.openai.resolve_auth_path(None)?;
            let account = config
                .openai
                .accounts
                .iter()
...
mod tests {
    ...
    fn status_lines_list_codex_accounts_and_flag_an_unmanaged_login() {
        ...
        assert!(lines.contains("Codex"), "{lines}");
